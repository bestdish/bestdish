/**
 * AI Scraper - Nationwide Chains (Small Batch)
 * Processes only 5 restaurants at a time to avoid API rate limits
 */
...
async function main() {
  console.log('🍽️  BestDish AI Scraper - Nationwide Batch')
  ...
  try {
    ...
    const restaurants = await prisma.restaurant.findMany({
      ...
      take: 5 // Only process 5 at a time to avoid API limits
    })
    
    console.log(`📊 Found ${restaurants.length} nationwide restaurants to process (batch of 5)`)
    ...
    const results = await processRestaurants(restaurants, restaurants.length, 3000) // 3 second delay between restaurants
