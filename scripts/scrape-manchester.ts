/**
 * AI Scraper - Manchester Only Test Run
 */
...
async function main() {
  console.log('🍽️  BestDish AI Scraper - Manchester Test')
  console.log('━'.repeat(50))
  console.log('')
  
  try {
    // Fetch Manchester city
    const manchester = await prisma.city.findFirst({
      where: {
        slug: 'manchester'
      }
    })
    ...
    if (!manchester) {
      console.error('❌ Manchester not found in database')
      process.exit(1)
    }
    
    // Fetch top 20 Manchester restaurants by rating
    ...
    const restaurants = await prisma.restaurant.findMany({
      ...
      take: 30 // Fetch extra to account for skips due to missing photos
    })
    
    console.log(`📊 Found ${restaurants.length} Manchester restaurants to process`)
    ...
    // Process restaurants (target 20 successes)
    const startTime = Date.now()
    const results = await processRestaurants(restaurants, 20, 2000) // Target 20, 2 second delay
